...
# 设置PyTorch默认的数据类型为float，确保后续所有张量默认以float类型创建
torch.set_default_dtype(torch.float)  

# 设置PyTorch的随机数种子为1234，这样每次运行代码时，涉及到的随机操作（如随机初始化等）结果可复现
torch.manual_seed(1234)  

# 设置numpy库的随机数种子为1234，保证numpy中随机相关操作结果的可复现性，方便调试和对比结果
np.random.seed(1234)  

# 根据系统是否有可用的GPU来配置计算设备，如果有GPU则使用'cuda'设备，否则使用'cpu'设备
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  
# 打印当前代码使用的计算设备信息，方便查看是在GPU还是CPU上运行
print(device)  

# 定义目标函数f_real，它接受两个参数x和t，返回按照数学表达式torch.exp(-t) * (torch.sin(np.pi * x))计算的结果
# 这个函数可能代表了某个实际的物理模型或者数学问题中的真实函数关系
def f_real(x, t):  
    return torch.exp(-t) * (torch.sin(np.pi * x))

# 以下是一个辅助函数，用于绘制等高线图和三维曲面图，将重复的绘图逻辑提取出来方便复用和代码简洁性
